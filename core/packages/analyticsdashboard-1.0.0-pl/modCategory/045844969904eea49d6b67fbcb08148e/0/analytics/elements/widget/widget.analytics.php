...
if (!empty($_POST['siteSelect'])) {
    ...
    echo '<META HTTP-EQUIV=Refresh CONTENT="0; URL='.$modx->getOption('manager_url').'">';
}

//retrieve profiles
$profilesxml = $ga->callApi($settings['sessionToken'], 'https://www.google.com/analytics/feeds/accounts/default');
$profiles = $ga->parseAccountList($profilesxml);
$modx->smarty->assign('profiles', $profiles);

//check if profile isset
if (!isset($continue) && empty($settings['profileId']) || empty($settings['accountId']) || empty($settings['webPropertyId'])){
